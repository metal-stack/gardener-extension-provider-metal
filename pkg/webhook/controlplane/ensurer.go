...
func additionalDNSConfFile(dnsServers []apimetal.NetworkServer) extensionsv1alpha1.File {
	...
	for _, dns := range dnsServers {
		...
		ip := dns.IP
		if dns.IPFamily == corev1.IPv6Protocol {
			ip = "[" + ip + "]"
		}

		resolveDNS += fmt.Sprintf("%s:%d#%s ", dns.IP, port, dns.Hostname)
	}
...
func additionalNTPConfFile(ntpServers []apimetal.NetworkServer) (extensionsv1alpha1.File, error) {
	var ntps string
	for _, ntp := range ntpServers {
		port := ntp.Port
		if port != 123 && port != 0 {
			return extensionsv1alpha1.File{}, fmt.Errorf("unsopported non-standard port %d for NTP server %q named %q", ntp.Port, ntp.Hostname, ntp.Name)
		}
		ntps += ntp.Hostname + " "
	}
